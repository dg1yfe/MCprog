...
if os.access('build/ptyserv', os.X_OK):
    ...
    # The fake radio serves eva9_real.bin and its ident says "5/6 Tone radio", so detection settles
    # on eva_56 -- which keeps a programming date at 0x0AF, not a write counter, and does not touch
    # it on write.  A 512-byte image whose ident said otherwise would move 0x0AF as well.
    check(diff and diff <= {0x000, 0x0E2, 0x0E3, 0x0E4},
